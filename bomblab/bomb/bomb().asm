...
0000000000400cbc <call_gmon_start>:
  ...
  400cc0:	48 8b 05 19 23 20 00 	mov    0x202319(%rip),%rax        # 602fe0 <__gmon_start__>；Q：这里有一些疑问，602fe0指向的是一条指令吗？为什么我找不到？
  400cc7:	48 85 c0             	test   %rax,%rax
  400cca:	74 02                	je     400cce <call_gmon_start+0x12> # if(!(*%rax==0)) call *rax add rsp 
...
0000000000400da0 <main>:
  ...
  400e37:	48 89 c7             	mov    %rax,%rdi # readline后将读入的参数的指针传给rax
...
0000000000400ee0 <phase_1>:
  ...
  400ee4:	be 00 24 40 00       	mov    $0x402400,%esi # 0x402400在这里是什么意思？有什么用？
  400ee9:	e8 4a 04 00 00       	call   401338 <strings_not_equal>
  400eee:	85 c0                	test   %eax,%eax
  400ef0:	74 05                	je     400ef7 <phase_1+0x17> # 如果返回0（即字符串相同），就返回
  400ef2:	e8 43 05 00 00       	call   40143a <explode_bomb>
  400ef7:	48 83 c4 08          	add    $0x8,%rsp # 这一步的意义是什么？
  400efb:	c3                   	ret    
...
000000000040131b <string_length>: # 实现将长度存到%rax中
  40131b:	80 3f 00             	cmpb   $0x0,(%rdi)  # cmp是用减法来实现的？取低的8位吗？
...
0000000000401338 <strings_not_equal>:
  ...
  401347:	41 89 c4             	mov    %eax,%r12d # r12d 第一个length
  40134a:	48 89 ef             	mov    %rbp,%rdi  
  40134d:	e8 c9 ff ff ff       	call   40131b <string_length> # 0x402400为起始地址的字符串的长度
  401352:	ba 01 00 00 00       	mov    $0x1,%edx # ？
  401357:	41 39 c4             	cmp    %eax,%r12d # 比较input字符串的长度和0x402400的长度是否相同
  40135a:	75 3f                	jne    40139b <strings_not_equal+0x63> # 不等->直接返回1
  40135c:	0f b6 03             	movzbl (%rbx),%eax
  40135f:	84 c0                	test   %al,%al # 检测rbx存储的地址中，低8位是否为‘\0’,0就je成立，返回0,即字符串相同
  401361:	74 25                	je     401388 <strings_not_equal+0x50> # 同上
  401363:	3a 45 00             	cmp    0x0(%rbp),%al # 如果不是，那么将两个字节进行比较
  401366:	74 0a                	je     401372 <strings_not_equal+0x3a> # 如果相同，那么rbx+1,进行移动，方便后续进行比较
...
00000000004015c4 <phase_defused>:
  ...
  4015cf:	00 00 
  4015d1:	48 89 44 24 68       	mov    %rax,0x68(%rsp) # 栈rsp+68位置设置为0x28
  4015d6:	31 c0                	xor    %eax,%eax # 低32位置0
  4015d8:	83 3d 81 21 20 00 06 	cmpl   $0x6,0x202181(%rip)        # 603760 <num_input_strings> 看是否有六个参数
  4015df:	75 5e                	jne    40163f <phase_defused+0x7b> # 如果不相同，
  ...
  40163f:	48 8b 44 24 68       	mov    0x68(%rsp),%rax # 如果不相同，0x28置于rax
  401644:	64 48 33 04 25 28 00 	xor    %fs:0x28,%rax # rax置于0
  40164b:	00 00 
  40164d:	74 05                	je     401654 <phase_defused+0x90> # rax置于0,跳转，结束
  40164f:	e8 dc f4 ff ff       	call   400b30 <__stack_chk_fail@plt> # 否则：？
