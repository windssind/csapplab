...
word f_pc = [
	...
	M_icode == IJXX && !M_Cnd : M_valA;## 即条件分支不跳转
	# Completion of RET instruction
	W_icode == IRET : W_valM;
	# Default: Use predicted value of PC
	1 : F_predPC;## 是不是还缺了jmp和call？不过也没有所谓，ncopy.ys没有用到call
];
...
bool F_stall =
	...
	IRET in { D_icode, E_icode, M_icode };## 这个可以理解
...
bool E_stall = 0;
bool E_bubble =
	# Mispredicted branch ##不需要预测就插进去泡泡
	(E_icode == IJXX && !e_Cnd) ||
	# Conditions for a load/use hazard
	E_icode in { IMRMOVQ, IPOPQ } &&
	 E_dstM in { d_srcA, d_srcB};
